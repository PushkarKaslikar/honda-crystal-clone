...
export default function QuickActions({ onOpenBooking }) {
  ...
  return (
    <section className="bg-slate-50 py-10 px-6 md:px-12 relative z-20">
          ...
          {actions.map((act) => {
            ...
            return (
              <button
                key={act.title}
                onClick={act.action}
                className="group relative flex flex-col text-left p-6 bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-300 rounded-2xl transition-all duration-300 transform hover:-translate-y-1 active:translate-y-0 shadow-sm hover:shadow-md"
              >
                ...
                <div className="w-10 h-10 rounded-xl bg-slate-100 group-hover:bg-honda-red text-slate-600 flex items-center justify-center mb-4 transition-colors duration-300">
